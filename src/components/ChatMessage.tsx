...
const MessageDisplay = ({userID, displayName, time, content, profilePicture, showAvatar, showSpacer}
  : {
    userID : string,
    displayName: string,
    time: string,
  ...
  return (
    <div
      className={showSpacer ? 'chat-message-container-spaced' : 'chat-message-container'}>
        ...
        {showAvatar ? <p className='chat-message-display-name'>{userID == '0' ? "Deleted user" : displayName}</p> : <></>}
