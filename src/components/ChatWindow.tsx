...
const ChatWindow = forwardRef<ChatWindowRef, ChatWindowProps>((props, ref) => { // Wrap the component with forwardRef so the parent can pass a ref;  useImperativeHandle exposes methods to that ref
    ...
    useImperativeHandle(ref, () => ({ // Think of this as a more complex version of a public method in C#
        scrollToBottom: () => { // this will, once called, animate a scroll to bottom animation
    ...
    useLayoutEffect(() => { // Runs before the screen gets rendered but after the screen gets painted
        const el = scrollContainerRef.current;
        if (!el) return;

        const handleScroll = () => { // sets wasAtBottom to a small pixel-perfect check. Probably should change in the future
            wasAtBottom.current = 
    ...
    useLayoutEffect(() => { // Actually handle the scrolling
        const el = scrollContainerRef.current;
        if (!el) return;
    ...
    return (
        ...
        return (
            <MessageDisplay
                ...
                time={message.messageTime.toLocaleString(
                    undefined,
                    {
                        dateStyle: new Date().getDate() != message.messageTime.getDate() ? 'medium' : undefined,
                        timeStyle: 'short'
                    }
                )}
                content={message.userContent} showAvatar={showAvatar} showSpacer={showSpacer}>
            </MessageDisplay>
        )})}
        </div>
    )
